...
`--force` does not lift the deferral, because it authorizes discarding unlanded work, never the captain's question, and `answer` remains the only act that resolves the call by either closing the question or releasing the gated work.
