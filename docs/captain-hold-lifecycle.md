...
It requires a non-empty captain decision file of at most 8192 bytes and writes a resolution block carrying the decision digest and a `Resolution mode:` at the top of the task body, with the previous body preserved below the block and archived through tasks-axi `--archive-body`.
The default close delegates to `bin/fm-backlog-transition-lib.sh` rather than bypassing its completion-provenance contract, while `--release` uses `tasks-axi unhold` so a captain-gated work item resumes instead of closing.
...
On 0 only the final state transition changes: after cleanup and still under the task's own lock, teardown records one `Deliverable of the finished work: ...` line at the end of the task body and runs `tasks-axi reopen`, so the row returns to Queued with its hold intact and lands in Captain's Call instead of reading as work still under way.
