...
- A test owns a temporary home and never creates, removes, or modifies a path under the repository checkout; `bin/fm-test-run.sh` refuses one that does, and [`docs/fm-checkout-write-guard.md`](../../../docs/fm-checkout-write-guard.md) owns the reasoning and the pinned pre-existing exceptions.
