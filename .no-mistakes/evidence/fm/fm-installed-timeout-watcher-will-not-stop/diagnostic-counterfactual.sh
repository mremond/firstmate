...
. "/Users/mremond/.no-mistakes/worktrees/acf4a767348a/01M2FG7SEEP1VBZ3B5SX35BJ6Q/tests/lib.sh"
...
test_returned_custom_check_descendants_are_drained() {
  ...
  for backend in fallback-timeout; do
...
#!/usr/bin/env bash
perl -e '$SIG{TERM}="IGNORE"; if (!fork()) { select undef, undef, undef, 60; exit 0; } open my $ready, ">", $ENV{FM_TEST_DESCENDANT_READY} or die $!; print {$ready} "ready\n"; close $ready; select undef, undef, undef, 60; open my $sentinel, ">", $ENV{FM_TEST_DESCENDANT_SENTINEL} or die $!; print {$sentinel} "late\n"; close $sentinel; select undef, undef, undef, 1' &
...
WATCH=/Users/mremond/.no-mistakes/worktrees/acf4a767348a/01M2FG7SEEP1VBZ3B5SX35BJ6Q/.test-phase/diagnostic-mutant/bin/fm-watch.sh
test_returned_custom_check_descendants_are_drained
