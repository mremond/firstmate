...
LAB=ROOT/'.test-phase/live-r2'
LAB.mkdir(parents=True,exist_ok=True)
BASH='/opt/homebrew/bin/bash'
results=[]
procs=[]
log=(EV/'live-watcher-r2.log').open('w')
def say(msg):
...
record('TERM stops the watcher, publishes downtime, and releases the singleton',normal)
...
record('Held marker lock cannot prevent stop; holder is preserved and the next watcher recovers',held)
...
record('Unwritable state makes acquisition return and watcher shutdown reach its deadline',resource)
...
'''
 ...
 assert 'check: rearm-resurface' in out and 'already running' not in out
record('A stealer killed after removal is reclaimed and watcher startup still works',abandoned)
...
record('Returned authenticated check descendant is drained by the real fallback timeout path',descendant)

for p in procs:
 if p.poll() is None:
  p.kill(); p.wait()
(EV/'live-watcher-results-r2.json').write_text(json.dumps(results,indent=2)+'\n')
log.close()
