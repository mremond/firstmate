...
run_watcher_bounded() {
  ...
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 10; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
    env FM_HOME="$home" FM_ROOT_OVERRIDE="$watch_root" FM_CHECK_INTERVAL="$check_interval" FM_CHECK_TIMEOUT="$check_timeout" \
      FM_POLL=0.02 FM_HEARTBEAT=999999 FM_SIGNAL_GRACE=0 PATH="$PATH" "$WATCH" "$@"
}
...
#!/usr/bin/env bash
perl -e '$SIG{TERM}="IGNORE"; my $g=fork; die $! unless defined $g; if (!$g) {sleep 60; exit} open my $gf, ">", $ENV{NM_GRANDCHILD} or die $!; print {$gf} "$g\n"; close $gf; open my $ready, ">", $ENV{FM_TEST_DESCENDANT_READY} or die $!; print {$ready} "ready\n"; close $ready; sleep 60' &
...
SH
    ...
    FM_HOME="$dir/home" FM_ROOT_OVERRIDE="$ROOT" FM_POLL=0.1 FM_CHECK_INTERVAL=999999 \
      ...
      FM_TEST_DIRECT_DONE="$direct_done" PATH="$PATH" "$WATCH" \
      > "$dir/watch.out" 2> "$dir/watch.err" &
    watcher_pid=$!
    i=0
    while [ "$i" -lt 200 ]; do
    ...
    while [ "$i" -lt 150 ]; do
      watcher_state=0
      PATH="" is_live_non_zombie "$watcher_pid" || watcher_state=$?
