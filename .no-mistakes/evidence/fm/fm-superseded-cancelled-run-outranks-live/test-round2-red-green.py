...
failures = []
with (EVIDENCE / 'test-round2-red-green.log').open('w') as log:
    ...
    for ref, names in cases.items():
        ...
        for name in names:
            for phase, product in [('RED', historical / 'bin/fm-crew-state.sh'), ('GREEN', ROOT / 'bin/fm-crew-state.sh')]:
                ...
                log.write(f'\n{phase} {name} against {ref if phase == "RED" else "622eb46201cb243e72bd2437c9fa76c90b38160b"}\n')
