...
meta.write_text(meta.read_text().replace('window=nm-validation-live:local-delivery','window=firstmate:fm-local-delivery'))
...
for state in ['dangling','unreadable-project','unreadable-user','absent','readable']:
    ...
    p=run([ROOT/'bin/fm-captain-hold.sh','hold','duplicate-id','--reason','Choose safely'],h,expect=None,env_extra={'FM_DATA_OVERRIDE':records},out='config-'+state+'.txt')
