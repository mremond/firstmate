...
for name,args in [('local',['fm-merge-local.sh','same-id']),('pr',['fm-pr-merge.sh','same-id','https://github.com/kunchenguid/firstmate/pull/4048'])]:
 ...
 env.update(FM_HOME=str(h),FM_ROOT_OVERRIDE=str(L/'code-root'),FM_STATE_OVERRIDE=str(h/'state'),FM_DATA_OVERRIDE=str(h/'data'),FM_CONFIG_OVERRIDE=str(h/'config'),FM_PROJECTS_OVERRIDE=str(h/'projects'),HOME=str(L/'user-home'),TMPDIR=str(R/'.test-phase-r2/tmp'),TMUX_TMPDIR=str(R/'.test-phase-r2/tmp'),FM_GATE_REFUSE_BYPASS='1')
 ...
 try:
  ...
  deadline=time.monotonic()+60;blocked=False
  while time.monotonic()<deadline:
   ...
   for line in lines:
    f=line.split(None,2)
    if len(f)==3 and f[1]==str(merge.pid) and Path(f[2]).name.strip('()')=='sleep': blocked=True
   if blocked: break
   time.sleep(.1)
