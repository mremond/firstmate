...
p=run([ROOT/'.test-local/pre-r2/bin/fm-captain-hold.sh','hold','duplicate-id','--title','Hold regression','--reason','Choose safely'],h,env_extra={'FM_DATA_OVERRIDE':records},out='pre-r2-wrong-backlog.txt')
assert before!=(h/'data/backlog.md').read_bytes() and configured_before==(records/'backlog.md').read_bytes()
