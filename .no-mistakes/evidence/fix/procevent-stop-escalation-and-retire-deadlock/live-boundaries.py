...
LOG=(EVIDENCE/'live-boundaries.log').open('w',buffering=1)
RESULTS=[]
...
def alive(pid):
    try: os.kill(pid,0); return True
        ...
        # Darwin can deny kill(0) during teardown; inspect actual group members
        # instead of treating a denied probe as proof either way.
        if pid > 0: return True
        groups=subprocess.check_output(['/bin/ps','-axo','pgid='],text=True)
        return str(-pid) in groups.split()
def ps(pid):
...
c=Case('target','first-signal-nonleader-fresh')
try:
    c.start()
    rows=c.claim.read_text().splitlines(); original=list(rows)
    identity=subprocess.check_output(['bash','-c','. "$1/bin/fm-wake-lib.sh"; fm_pid_identity "$2"','_',str(ROOT),str(c.child)],env=c.env,text=True).strip()
    rows[1]=str(c.child); rows[3]=identity
    c.claim.write_text('\n'.join(rows)+'\n');c.claim.chmod(0o600)
    log(f'[{c.id}] claim now identifies actual live nonleader: {ps(c.child)}')
    p,_=c.cmd('retire',c.id)
    result(c.id,p.returncode!=0 and alive(c.child) and alive(c.pid) and not c.signals() and c.source.exists() and c.claim.exists(),cli_status=p.returncode,runner_alive=alive(c.pid),nonleader_alive=alive(c.child),signals=c.signals(),registration_preserved=c.source.exists(),claim_preserved=c.claim.exists())
    c.claim.write_text('\n'.join(original)+'\n');c.claim.chmod(0o600)
finally:c.cleanup()

c=Case('base','abandoned-guard-proof',lease=5,tick=1)
try:
    c.start()
    rows=subprocess.check_output(['/bin/ps','-axo','pid=,command='],text=True).splitlines()
    guards=[int(r.split(None,1)[0]) for r in rows if '_owner-watchdog '+c.id+' ' in r]
    assert len(guards)==1, guards
    guard_pid=guards[0]
    log(f'[{c.id}] live guard before expiry: {ps(guard_pid)}')
    assert waitfor(lambda:bool(c.signals()),20)
    guard_gone=waitfor(lambda:not alive(guard_pid),12)
    result(c.id,guard_gone and not alive(c.pid) and alive(c.child),baseline_defect_reproduced=True,guard_gone=guard_gone,runner_gone=not alive(c.pid),child_alive=alive(c.child),signals=c.signals())
    log(f'[{c.id}] guard={ps(guard_pid) or "absent"}; runner={ps(c.pid) or "absent"}; survivor={ps(c.child)}')
finally:c.cleanup()
