...
ROOT=Path.cwd()
RUN=ROOT/'.local-procevent-validation/manual-v3'
...
def alive(pid):
    try: os.kill(pid,0); return True
        ...
        out=subprocess.check_output(['/bin/ps','-o','pid=','-g' if pid<0 else '-p',str(abs(pid))],text=True)
...
(EVIDENCE/'manual-results-v3.json').write_text(json.dumps(results,indent=2)+'\n')
