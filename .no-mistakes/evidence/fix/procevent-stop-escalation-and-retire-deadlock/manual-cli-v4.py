...
ROOT=Path.cwd()
RUN=ROOT/'.local-procevent-validation/manual-v4'
...
def alive(pid):
    try: os.kill(pid,0); return True
        ...
        # A vanishing macOS group can briefly reject kill(0); inspect its actual members.
        out=subprocess.run(['/bin/ps','-o','pid=','-g' if pid<0 else '-p',str(abs(pid))],capture_output=True,text=True).stdout
...
def start(name, lease='5',attached=False):
    ...
    wait_for(lambda: '/bin/sleep 180' in subprocess.run(['/bin/ps','-o','command=','-g',str(pid)],capture_output=True,text=True).stdout)
...
for name in os.environ.get('NM_MANUAL_CASES','reconcile-healthy,crashed-group,mismatched-live,nonleader-live').split(','):
    h=None
    try:
        h,pid,claim,launch,log=start(name,lease='600',attached=name=='reconcile-healthy')
        ...
        if name=='reconcile-healthy':
            ...
            time.sleep(10)
            assert alive(-pid) and claim.read_bytes()==original and source.exists()
            say('After ten one-second guard-check intervals: group still alive, claim unchanged.')
        else:
            ...
            if name=='mismatched-live':
                ...
                rows=subprocess.check_output(['/bin/ps','-o','pid=,command=','-g',str(pid)],text=True).splitlines()
                child=int(next(row.split()[0] for row in rows if row.strip().endswith('/bin/sleep 180')))
...
(EVIDENCE/'manual-results-v4.json').write_text(json.dumps(results,indent=2)+'\n')
