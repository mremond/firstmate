...
Give every test its own temporary home and never let it create, remove, or modify a path under the repository checkout: the checkout is shared with every other script and `bin/fm-test-run.sh` refuses a script that writes into it, for the reasons in [the checkout write guarantee](docs/fm-checkout-write-guard.md).
